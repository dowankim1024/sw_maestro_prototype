...
function buildUserPrompt(input: ChatReplyRequest): string {
  ...
  return [
    ...
    "- 사용자가 처음 듣는다고 생각하고 쉬운 말부터 쓰세요.",
    "- 어려운 단어는 피하고, 꼭 필요하면 바로 쉬운 말로 풀어쓰세요.",
...
function characterFallback(id: CharacterId, closing = false): string {
  ...
  switch (id) {
    case "kkang":
      return "쉽게 말하면 이 이슈가 내 생활에 어디 닿는지가 포인트야. 가격이든 시간이든 결국 티가 나는 데가 있거든.";
    case "uncle":
      return "쉽게 말하면 이 이슈는 한 가지 사건만 보면 놓치는 게 있어요. 주변 흐름까지 같이 봐야 이해가 더 쉬워집니다.";
    case "prof":
      return "쉽게 말하면 사람들이 지금 이걸 어떤 느낌으로 받아들이는지가 중요해. 다만 반응이랑 사실은 따로 봐야 해.";
    case "pm":
    default:
      return "쉽게 말하면 이 사안은 한 줄로 바로 단정하기 어려워. 먼저 출처에 나온 사실과 그 위에 붙은 해석을 나눠 봐야 해.";
  }
}
